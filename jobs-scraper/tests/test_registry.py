...
def test_source_kinds_matches_the_shipped_adapters():
    ...
    something has to notice when the two disagree. Only the kinds actually used
    by registry entries are checked, because ``sources/__init__.py`` does not
    register every module in the package.
    """
    from tactical_jobs.sources import available_kinds

    registered = set(available_kinds())
    for kind in used_kinds():
        assert kind in registered, f"{kind!r} is not a registered adapter"
...
def test_every_registry_entry_is_reachable_by_its_own_name():
    for employer in REGISTRY:
        assert match_employer(employer.name) is not None, employer.slug
        assert match_employer(employer.slug) is not None, employer.slug
...
def test_verification_hint_falls_back_for_an_unmapped_kind():
    assert verification_hint("some-future-vendor")
