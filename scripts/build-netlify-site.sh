...
if want_rev; then
  ...
  # The drafted grant applications, at /apply. Generated by
  # funding-scraper/applications/build.py; the daily routine adds a packet a
  # day. Optional like the feed: its absence must not fail the build.
  if [ -f "$SRC_APPLY" ]; then
    cp -f "$SRC_APPLY" "$OUT/apply.html"
  else
    echo "note: $SRC_APPLY not present; /apply will not be published."
  fi
fi
